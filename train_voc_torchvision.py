...
def train_one_epoch(model, optimizer, loader, scaler, device):
    ...
    return {
        ...
        "time": elapsed,
        "img_s": num_batches * loader.batch_size / elapsed,
    }
...
def parse_args():
    p = argparse.ArgumentParser(description="SSD300-VGG16 (torchvision) on VOC2007")
    ...
    p.add_argument("--output-dir", default="checkpoints_tv")
...
def main():
    ...
    train_ds = VOC2007Dataset(args.data_root, split="trainval", transform=SSDAugmentation())
    val_ds   = VOC2007Dataset(args.data_root, split="test",     transform=ValTransform())
    ...
    print("Building model (torchvision SSD300)...")
    weights_backbone = (
        None if args.no_pretrained_backbone
        else VGG16_Weights.IMAGENET1K_FEATURES
    )
    model = ssd300_vgg16(
        weights=None,
        num_classes=NUM_CLASSES,
        weights_backbone=weights_backbone,
        trainable_backbone_layers=3 if weights_backbone is not None else 5,
    ).to(device)
    ...
    if args.resume:
        ...
        print(f"  Resuming from epoch {start_epoch}  (best mAP: {best_map * 100:.2f}%)")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        torch.save(ckpt, os.path.join(args.output_dir, f"ssd_ep{epoch:03d}.pth"))
        if is_best:
